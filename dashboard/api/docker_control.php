<?php
header('Content-Type: application/json');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: GET, POST');
header('Access-Control-Allow-Headers: Content-Type');

$action = isset($_GET['action']) ? $_GET['action'] : null;
$id = isset($_GET['id']) ? escapeshellarg($_GET['id']) : null;
$name = isset($_GET['name']) ? escapeshellarg($_GET['name']) : null;
$image = isset($_GET['image']) ? escapeshellarg($_GET['image']) : null;

switch ($action) {
    case 'start':
        $output = shell_exec("docker start $id 2>&1");
        echo json_encode([
            'success' => strpos($output, 'Error') === false,
            'error' => strpos($output, 'Error') !== false ? $output : null
        ]);
        break;
    case 'stop':
        $output = shell_exec("docker stop $id 2>&1");
        echo json_encode([
            'success' => strpos($output, 'Error') === false,
            'error' => strpos($output, 'Error') !== false ? $output : null
        ]);
        break;
    case 'delete':
        $output = shell_exec("docker rm $id 2>&1");
        echo json_encode([
            'success' => strpos($output, 'Error') === false,
            'error' => strpos($output, 'Error') !== false ? $output : null
        ]);
        break;
    case 'create':
        $output = shell_exec("docker run -d --name $name $image 2>&1");
        echo json_encode([
            'success' => strpos($output, 'Error') === false,
            'error' => strpos($output, 'Error') !== false ? $output : null
        ]);
        break;
        default:
        // Überprüfen, ob eine ungültige Aktion angefragt wurde
        if (isset($_GET['action']) && !in_array($_GET['action'], ['start', 'stop', 'delete', 'create'])) {
            echo json_encode([
                'error' => 'Ungültige Aktion',
                'available_actions' => ['start', 'stop', 'delete', 'create'] // Liste der erlaubten Aktionen
            ]);
            break; // Beende die Verarbeitung hier
        }
    
        // Standardfall: Docker-Informationen zurückgeben
        $docker_status = shell_exec("systemctl is-active docker");
        $docker_status = trim($docker_status) === 'active' ? 'running' : 'stopped';
    
        $docker_logs = shell_exec("journalctl -u docker.service --since '1 day ago' -p err -n 25");
        $output = shell_exec("docker ps -a --format '{{.ID}}|{{.Names}}|{{.Status}}'");
        $containers = array_filter(explode("\n", trim($output)));
    
        echo json_encode([
            'docker_status' => $docker_status,
            'docker_logs' => $docker_logs,
            'docker' => $containers
        ]);
        break;
    
}
